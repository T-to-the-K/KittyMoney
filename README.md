...
- **Kitty groups** — create a group with a name and a **monthly pot** (the amount handed to a full-share collector each month), or go free-form "any amount".
...
- **Fixed payments from the pot** — the monthly pot fixes each member's payment by their share: full share pays (pot ÷ total shares), half pays half, and each month's contributions add up to exactly the pot. The app **rejects** any amount that doesn't match.
...
- **Payments** — tick each member off once they contribute to a month (fixed amount for pot kitties, or any amount in free-form mode).
